...
case class ParseFuncs[T](
  fromString: String => List[T],
  fromBytes: (Array[Byte], Int) => List[T])
...
case class RestResponse[T](status: Int, bytes: Array[Byte], size: Int, statusMessage: String, var parsedVals: List[T]) {
